...
Batch discipline: **confirm the template mapping with the user before building any design.** First list which `MB -` template each post maps to and **wait for the user to confirm or redirect** — don't create the designs off your own pick. Where more than one `MB -` template fits a post, present the options and ask which (e.g. witness → `MB - Video Witness Ad` 9:16 story vs `MB - Witness Ad_129x188mm_PRINT` print; quote → square vs LinkedIn landscape; review → feed vs story). Then **fit each post's copy to its template — flag any too long to fit cleanly**. **Never invent a design** and never fall back to a `Union -`/`CFA` template or `generate-design` — if no `MB -` template fits a post, say so and ask.
