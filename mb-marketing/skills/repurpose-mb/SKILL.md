...
If the user wants creatives, draft them on real MB brand templates with the copy **already placed** — follow `post-writer-mb`'s Step 8 flow exactly (create a new design from the template → open the design for editing → **name the design** `PRACTICE AREA - REQUEST - DDMMYYYY` via `update_title` → `replace_text` the copy fitted to the layout → attach a supplied image via `upload-asset-from-url` + `update_fill` where the template has an image element → render the thumbnail → commit → send the `edit_url` + thumbnail for review). **Never edit the brand template itself; always work on the new design created from it.** **Canva tool set (verified 2026-08-09):** `create-design-from-brand-template` → `read-design` (`open_transaction: true`) → `edit-design` (apply `operations`, then `finalize: "commit"`/`"cancel"`). This replaced the older `start-editing-transaction`/`perform-editing-operations`/`commit-editing-transaction` tools; if a named tool 404s, search `mcp__claude_ai_Canva__*` and use the current equivalent — the create → read+open → edit → commit sequence is what matters.

**Pick by the `MB -` title-prefix rule** (MB's Canva uses a three-way naming convention, verified 2026-08-08). Filter `search-brand-templates` results to titles starting with **`MB - `** — those are standalone Maurice Blackburn. **`Union -`** = MB×AWU co-brand with a *locked* AWU partnership footer (use only for a genuine MB+AWU union/EBA post, confirm first). **`CFA`** = Claims Funding Australia, a separate brand — never for an MB post. Route by **topic first**, then format:
- **Witness ad** → present all four and let the user pick: `MB - Video Witness Ad` (9:16 video), `MB - Witness Ad_129x188mm_PRINT`, `MB - Witness Ad_134x188mm_PRINT` (print), `MB - Abuse Witness Ad_1920x1080px` (16:9 video, abuse appeals).
- **Class action** → present all three, let the user pick: `MB - Class Action Template - Image - Instagram` (image-capable — headline + "link in bio" CTA + background image slot; use when a banner/photo is supplied), `MB - Class Action Template - No Image - Instagram` (text-only), `MB - Quote - x 3 versions` (pull-quote style).
- **Anything else** → best-fit `MB -` template: `MB - Quote - x 3 versions` (square) / `MB - Quote - LinkedIn Landscape 1200x627`; `MB - 5 star Google review - White` (feed square, or 450×800 story) for testimonial/review; `MB - FF Awards template - with photo` for award/milestone; `MB - Name of Event` for event promo.

Not a hard list: filtering live by the `MB -` prefix picks up any new MB template automatically. **Connector caveat:** the Connect API returns only a subset — some (`Witness Ad_134x188mm`, `Abuse Witness Ad`, `Name of Event`) may not come back yet, and the Abuse ad is flattened (no editable text). If a confirmed template isn't returned or has no editable text, don't swap or invent — tell the user and offer the manual `create_url`/`view_url` or a reachable witness template instead. Never fall back to a `Union -`/`CFA` template or `generate-design`.

**QA every creative.** After each creative is committed, run **`qa-creative-mb`** on its `design_id` — WCAG 2.2 AA check (contrast, legible size, alt text) + a visual sanity read, with safe auto-fixes offered as a before/after the user approves (brand colours always win on contrast). Don't skip it.

Batch discipline: **confirm the template mapping with the user before building any design.** First list which `MB -` template each post maps to and **wait for the user to confirm or redirect**; where more than one fits, present the options and ask. Then **fit each post's copy to its template — flag any too long to fit cleanly**. Each created design gets its own `PRACTICE AREA - REQUEST - DDMMYYYY` title (confirm it with the user before applying; get today's date via `TZ="Australia/Sydney" date "+%d%m%Y"`).
