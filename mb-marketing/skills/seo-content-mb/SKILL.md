...
6. **Publish-ready page draft.** Deliver as an **HTML file** (`<slug>.html`), and it must actually be MB-branded, not bare unstyled markup, that has shipped broken before, do not repeat it:
   - Draft the page body markup: a metadata block as an HTML comment at the very top (labelled "for CMS/dev handoff, not rendered site copy"), containing Meta Title, Meta Description, Keywords as a Primary/Secondary list, People Also Asked, and Internal Links, mirroring the Targeting brief exactly, then the real page markup: breadcrumb nav, single H1, byline, question-based H2/H3 sections, answer-first paragraphs, tables, internal links using the real URLs from the Targeting brief, and the extractable disclaimer + jurisdiction line in body copy. **Every heading must be a real `<h1>`/`<h2>`/`<h3>` tag, never a styled `<p>`, `<div>`, or `<strong>` standing in for one.**
   - Build a JSON object with `meta_title`, `meta_description`, and `body_html` (everything you just drafted, comment block included), write it with `Write`, then run `python3 scripts/render_mb_html.py <input.json> <slug>.html`. This wraps the body in MB's real brand CSS (colours, Poppins, Reader Pro where the licensed font file is present, a serif fallback where it isn't) and the real MB logo, inline and self-contained, no external file dependencies except the public Poppins Google Font. **Never hand-write the `<head>`/`<style>` yourself or skip this script**, an unstyled or improvised-CSS HTML file is not an acceptable deliverable.
   - This file is the layout/visual overview and must look genuinely on-brand when opened in a browser, not just be semantically valid.
