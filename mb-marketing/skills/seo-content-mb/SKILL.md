...
2. **Read `brand-mb/reference/blog-voice-reference.md`** (the long-form voice anchor, built from ~50 real MB blogs). This is the concrete standard the mandatory voice pass (Step 4.5) grades against, reading it up front is what stops the draft coming out generic. If it isn't present, say so and fall back to `tone-of-voice.md`, but flag that the long-form anchor is missing.
...
6. **Publish-ready page draft.** Deliver as an **HTML file** (`<slug>.html`), and it must actually be MB-branded, not bare unstyled markup, that has shipped broken before, do not repeat it:
   - Draft the page body markup using these standard classes (the script below already styles them, do not invent alternative classes or inline styles):
     ...
     - Breadcrumb `<nav aria-label="Breadcrumb">`, single `<h1>`, `<p class="byline">` (name + role + credential + a link to the author's `/our-lawyers/` page, the by-line detail is a real GEO signal the reviewer values, so make it substantive, not just a name).
     ...
     - `<div class="faq-item">` per Q&A pair in the FAQ section, `<div class="related-card">` per related-article link.
